...
router.get("/serviciosenelmes", (req, res) => {
  db.sepelioSequelize
    .query(
      `
        select count(*) as 'cant'
        from servicios
        where fecha_recepcion between '${moment()
          .startOf("month")
...
router.get("/acumuladoliqop", (req, res) => {
  db.sepelioSequelize
    .query(
      `
        SELECT
...
router.get("/acumuladogastocaja", (req, res) => {
  db.sepelioSequelize
    .query(
      `
        select sum(total) as 'total'
        from gastos_caja
        where fecha between '${moment()
          .startOf("month")
          .format("YYYY-MM-DD")}' and '${moment()
        .endOf("month")
        .format("YYYY-MM-DD")}'
